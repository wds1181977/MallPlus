...
public class GoodsListAdapter extends BaseQuickAdapter<GoodsListModel.GoodsListBean, BaseViewHolder> {
    ...
    @Override
    protected void convert(BaseViewHolder holder, GoodsListModel.GoodsListBean data) {
        if(data == null) return;
        try {
            ...
            if (data.getPrimary_pic_url() != null) {
                Glide.with(holder.itemView.getContext()).load(data.getPrimary_pic_url()).apply(options).into(goodsImage);
            }
