...
public class AllOrderFragment extends XBaseFragment implements OnRefreshListener, OnLoadMoreListener, PayFragment.PayObjListener {
        ...
        adapter.set_OnItemClickListener(new OrderAdapter.OnItemClickListener() {
            ...
            public void onItemClickOne(View view, int position) {
                ...
                if (orderListBean.getOrderStatus() == 0) {
                    //未付款---取消订单
                    showPromptDialog(1, orderListBean.getId());
                }
            }
            ...
            public void onItemClickTwo(View view, int position) {
                ...
                } else if (orderListBean.getOrderStatus() == 202) {
                    showPromptDialog(2, orderListBean.getId());
                }
            }
    ...
    private void showPayDialog(DBMallManager.DBMallPayParams payParams) {
        ...
        manager.beginTransaction().remove(mPayFrgment).commit();
        mPayFrgment.show(manager, "mPayFrgment");
    }
     ...
     * 确认收货
     */
    private void confimAcceptOrder(String orderId) {
        ServerApi.getInstance().confimAcceptOrder(getContext(), orderId)
    ...
    private void getOrderList() {
        ServerApi.getInstance().getOrderList(getContext(), pageCount, "")
                .subscribeOn(Schedulers.io())
                .doOnSubscribe(new Consumer<Disposable>() {
                    @Override
                    public void accept(@NonNull Disposable disposable) throws Exception {
     ...
     * @param orderId
     */
    private void showPromptDialog(int typeFlag, String orderId) {
        String contentString = "";
        switch (typeFlag) {
            case 1:
                contentString = cnt.getResources().getString(R.string.are_you_sure_you_want_to_cancel_the_order);
                break;
            case 2:
                contentString = cnt.getResources().getString(R.string.determine_the_receipt);
                break;
        }
